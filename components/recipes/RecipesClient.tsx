...
export default function RecipesClient({
  ...
  useEffect(() => {
    const isModalOpen =
      (canCreate && (formOpen || Boolean(editingRecipe))) ||
  ...
  }, [canCreate, formOpen, editingRecipe, selectedRecipe, zoom]);
  ...
  async function createRecipe(data: RecipeInput) {
    if (!canCreate) {
      throw new Error("No create permission");
    }
  ...
  const isFormVisible = canCreate && (formOpen || Boolean(editingRecipe));
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed border-white/15 bg-white/[0.04] p-8 text-center">
          ...
          <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-slate-400">
            {canCreate
              ? "Create your first recipe with a title, cover image and preparation steps. You can add or remove details later."
              : "Recipes created by the owner will appear here."}
          </p>
          {canCreate && (
            <button
              type="button"
